...
def construct_model(model, fc1in_width=3072, fc1out_width=1024):
    if model == 'OneMLP_train':
        width = fc1out_width
        model = torch.nn.Sequential(OrderedDict([
            ('flatten', torch.nn.Flatten()),
            ('linear0', torch.nn.Linear(fc1in_width, width)),
            # ('relu0', torch.nn.ReLU())]))
            ('sig', torch.nn.Sigmoid())
            ]))
    elif model == 'OneMLP_test':
        width = fc1out_width
        model = torch.nn.Sequential(OrderedDict([
            ('flatten', torch.nn.Flatten()),
            ('linear0', torch.nn.Linear(fc1in_width, width)),
...
''' MLP '''
class MLP(nn.Module):
    def __init__(self, fc1in_width, fc1out_width, num_classes, act_layer='ReLU', dropout_p=0):
        ...
        self.model = torch.nn.Sequential(OrderedDict([
            ('flatten', torch.nn.Flatten()),
            ('linear0', torch.nn.Linear(fc1in_width, fc1out_width)),
            ('dp', dp(p=dropout_p)),
            ('relu0', act()),
            ('linear1', torch.nn.Linear(fc1out_width, 2*fc1out_width)),
            ('dp', dp(p=dropout_p)),
            ('relu1', act()),
            ('linear2', torch.nn.Linear(2*fc1out_width, 3*fc1out_width)),
            ('dp', dp(p=dropout_p)),
            ('relu2', act()),
            ('linear3', torch.nn.Linear(3*fc1out_width, 2*fc1out_width)),
            ('dp', dp(p=dropout_p)),
            ('relu3', act()),
            ('linear4', torch.nn.Linear(2*fc1out_width, fc1out_width)),
            ('dp', dp(p=dropout_p)),
            ('relu4', act()),
            ('linear5', torch.nn.Linear(fc1out_width, num_classes))
            ]))
...
class ConvNet(nn.Module):
    def __init__(self, channel, num_classes, net_width, net_depth, net_act, net_norm, net_pooling, im_size = (32,32), p=0):
    ...
    def _get_normlayer(self, net_norm, shape_feat):
        ...
        if net_norm == 'batchnorm':
            return nn.BatchNorm2d(shape_feat[0], affine=True, momentum=1)
